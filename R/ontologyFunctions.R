########################################
## Below are a series of functions cluster and 
## visualize pathway data
########################################

## Function to cluster categories by protein membership
## takes a named list category members of and returns their overlap,
## a plot, and a tentative clustering by dynamic tree cut.
...
findCategoryOverlap <- function(membershipList, similarityCut = 0.5){
  ...
  return(list(overlapMatrix = OG, namesMap = namesMap, plot = tilePlot, 
...
summarizeCategoryClusters <- function(catID, catName, catScore, catFDR, catSize, cluster){
  ...
  summaryPlot <- summaryTable %>%
    ...
    ggplot(aes(x = catSize, y = catScore, label = myLabeller(catName))) +
    geom_point() +
    geom_text_repel() +
    labs(x = "Number of proteins", y = "Score")
